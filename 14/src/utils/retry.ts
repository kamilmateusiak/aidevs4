...
export async function withPoll<T>(
  ...
  { maxRetries = 10, delay = 10000 }: { maxRetries?: number; delay?: number } = {}
): Promise<T> {
  for (let i = 0; i < maxRetries; i++) {
    const result = await fn();
    if (!shouldRetry(result)) return result;
    console.log(`  [poll] attempt ${i + 1}/${maxRetries}, retrying in ${delay}ms...`);
    await wait(delay);
  }
  throw new Error("Polling exceeded max retries.");
}
